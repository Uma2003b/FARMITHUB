...
crop_planning_bp = Blueprint('crop_planning', __name__,
                            template_folder=os.path.join('Crop_Planning', 'templates'),
                            static_folder=os.path.join('Crop_Planning', 'static'))

CORS(crop_planning_bp)
...
def predict():
    try:
        ...
        crop_planning_bp.logger.error(f"Prediction error: {str(e)}")
