...
forum_loan_bp = Blueprint('forum_loan', __name__)

CORS(forum_loan_bp, resources={r"/*": {"origins": "http://127.0.0.1:5500"}})
...
@forum_loan_bp.route('/api/firebase-config')
...
@forum_loan_bp.route('/process-loan', methods=['POST'])
def process_loan():
    try:
...
3. Highlight areas of strength and areas of weakness.
...
@forum_loan_bp.route('/forum', methods=['GET'])
def get_forum_posts():
    """Get all forum posts"""
    return jsonify(forum_posts)

@forum_loan_bp.route('/forum', methods=['POST'])
def add_forum_post():
